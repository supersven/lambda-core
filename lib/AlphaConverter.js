...
module.exports.alphaConverter = function AlphaConverter() {
    ...
    function convertInternal(ast, symbolMap) {
        if (Helpers.isLambda(ast)) {
            ...
            var lambdaDefinition = new Parser.LambdaDefinition(newParameterName, newExpression);
            ...
            var name = ast.get("x");
            var variable = new Parser.Variable(createID(name, symbolMap));
            ...
            return resultWithState(new Parser.Application(leftConverted.get("ast"), rightConverted.get("ast")), rightConverted.get("symbolMap"));
