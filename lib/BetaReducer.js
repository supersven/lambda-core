...
// todo - use persistent data structure semantics here: no new required
            return AstNodeFactory.LambdaDefinition(expression.get("parameters"), self.replaceVariableRecursive(expression.get("expression"), id, replacement));
        }
...
// todo - use persistent data structure semantics here: no new required
            return AstNodeFactory.Application(self.replaceVariableRecursive(left, id, replacement), self.replaceVariableRecursive(right, id, replacement));
        }
    ...
    self.reduceNextRedex = function (ast) {
        if (Helpers.isApplication(ast)) {
            ...
            if (Helpers.isLambda(left)) {
                ...
                return AstNodeFactory.Application(self.reduceNextRedex(left), right);
            }
        } else if (Helpers.isLambda(ast)) {
            var lambdaNode = ast;
            return AstNodeFactory.LambdaDefinition(lambdaNode.get("parameters"),self.reduceNextRedex(lambdaNode.get("expression")))
        } else {
            return ast;
        }
    };
